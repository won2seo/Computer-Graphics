...
GLfloat light_position_sunrise[] = { 0.0f, 5.0f, 10.0f, 0.0f }; // 아침 햇빛 위치
GLfloat light_position_sunset[] = { -5.0f, 1.0f, 3.0f, 0.0f };  // 석양 초기 위치

// 석양 애니메이션 변수
float sunset_light_angle = 0.0f; // 석양 각도 (0도에서 90도로 이동)
bool isAnimatingSunset = false; // 애니메이션 활성화 여부
bool isSunrise = true;          // 아침 햇빛 상태
...
void DrawSignWithTexture() {
    ...
    glTexCoord2f(1.0f, 1.0f); glVertex3f(0.5, 0.0, 0.0);
    glTexCoord2f(1.0f, 0.0f); glVertex3f(0.5, 1.0, 0.0);
    glTexCoord2f(0.0f, 0.0f); glVertex3f(-0.5, 1.0, 0.0);
    glEnd();
...
void Keyboard(unsigned char key, int x, int y) {
    if (key == 'h') {
        ...
        printf("'h': 도움말 표시\n");
        printf("'s': 석양 모드로 변경\n");
        printf("'d': 아침 햇빛 모드로 변경\n");
    }
    else if (key == 's') {
        ToggleLighting(); // 석양 모드 활성화
    }
    else if (key == 'd') {
        ToggleLighting(); // 아침 모드 활성화
    }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Park with Sunset Animation");
